...
function _chart(ConnectedScatterplot,labelledjobstats,width){return(
ConnectedScatterplot(labelledjobstats, {
  x: d => d.count,
  y: d => d.jobid,
  title: d => d.model,
  orient: d => d.side,
  yFormat: ".2f",
  xLabel: "count  (Avg per Chain) →",
  yLabel: "↑ jobid (avg) ",
  width,
  height: 750,
...
function _labelledjobstats(FileAttachment){return(
FileAttachment("labelledjobstats.csv").csv({typed: true})
)}
...
altplot(`Plot.plot({
  grid: true,
  x: {label: "count driven (per capita per model) →"},
  y: {label: "↑ Price of jobid (average per gallon, adjusted)"},
  marks: [
    Plot.line(labelledjobstats, {x: "count", y: "jobid", curve: "catmull-rom"}),
    Plot.dot(labelledjobstats, {x: "count", y: "jobid", fill: "currentColor"}),
    Plot.text(labelledjobstats, {filter: d => d.model % 5 === 0, x: "count", y: "jobid", text: "model", dy: -8})
  ]
})`)
)}
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["labelledjobstats.csv", {url: new URL("./labelledjobstats.csv", import.meta.url), mimeType: null, toString}]
  ]);
  main.builtin("FileAttachment", runtime.fileAttachments(name => fileAttachments.get(name)));
  main.variable(observer("viewof replay")).define("viewof replay", ["Inputs","chart"], _replay);
  main.variable(observer("replay")).define("replay", ["Generators", "viewof replay"], (G, _) => G.input(_));
  main.variable(observer("chart")).define("chart", ["ConnectedScatterplot","labelledjobstats","width"], _chart);
  main.variable(observer("labelledjobstats")).define("labelledjobstats", ["FileAttachment"], _labelledjobstats);
